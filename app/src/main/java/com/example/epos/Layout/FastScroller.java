package com.example.epos.Layout;
...
public class FastScroller extends LinearLayout {
    ...
    private void setRecyclerViewPosition(float y) {
        ...
        if (recyclerView != null) {
            ...
            targetPos = getValueInRange(0, itemCount - 1, (int) (proportion * (float) itemCount));
            recyclerView.scrollToPosition(targetPos);


            TextView text = (TextView) findViewById(R.id.fastscroller_handle);
            ProductAdapter adapter = (ProductAdapter) recyclerView.getAdapter();

            try {
                if (targetPos < adapter.getItemCount()) {
                    text.setText(String.valueOf(adapter.getItem(targetPos).getName().charAt(0)));
                }
